<template>
  <div class="min-h-screen bg-black flex flex-col items-center justify-center p-4">
    <!-- Spotify Logo -->
    <div class="mb-8">
      <svg viewBox="0 0 1134 340" class="h-[40px] text-white">
        <title>Spotify</title>
    ...
    <!-- Login Form -->
    <div class="w-full max-w-md bg-[#121212] p-8 rounded-lg text-center">
      <h1 class="text-white text-3xl font-bold mb-8">Log in to Spotify</h1>
      
      <button 
        @click="login" 
        class="w-full bg-[#1DB954] text-white font-bold py-3 px-6 rounded-full hover:bg-[#1ed760] transition-colors duration-200 flex items-center justify-center space-x-2"
      >
      ...
      <div class="mt-8 text-gray-400 text-sm">
        <p>Don't have an account? 
          <a href="https://www.spotify.com/signup" target="_blank" class="text-white hover:underline">Sign up for Spotify</a>
        </p>
      </div>
    </div>
...
function login(){
  window.location.href=`${AUTH_URL}?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=${encodeURIComponent(SCOPES)}&show_dialog=true`;
}
</script>
